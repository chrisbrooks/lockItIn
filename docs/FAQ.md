...
Unfortunately, scripts in package.json can't be commented inline because the JSON spec doesn't support comments, so here's info on what each script in package.json does here.
...
| lint:tools | Runs ESLint on build related JS files. (eslint-loader lints app files via webpack when `npm start` is run) |
| clean-dist | Removes everything from the dist folder. |
| remove-dist | Deletes the dist folder. |
| create-dist | Creates the dist folder and the necessary subfolders. |
| build:html | Adds raygun tracking script and copies to /dist. |
| prebuild | Runs automatically before build script (due to naming convention). Cleans dist folder, builds html, and builds LESS. |
...
│   └── styles                # CSS Styles, typically written in LESS
├── scripts                    # Node scripts that run build related tasks
...
|less-loader| Adds LESS support to Webpack|
...
Webpack serves your app in memory when you run `npm start`. No physical files are written.
However, the web root is /app, so you can reference files under /app in index.html.
When the app is built using `npm run build`, physical files are written to /dist and the app is served from /dist.

### How is LESS being converted into CSS and landing in the browser?
Magic! Okay, more specifically, we're handling it differently in dev (`npm start`) vs prod (`npm run build`)

When you run `npm start`:

 1. The less-loader compiles LESS into CSS
...
When you run `npm run build`:
 ...
 1. The less-loader compiles LESS into CSS
 2. The [extract-text-webpack-plugin](https://github.com/webpack/extract-text-webpack-plugin) extracts the compiled LESS into styles.css
 3. buildHtml.js adds a reference to the stylesheet to the head of index.html.

For both of the above methods, a separate sourcemap is generated for debugging LESS in compatible browsers.
...
All tests are placed in files that end in .spec.js. Spec files are placed in the same directory as the file under test. Why?
